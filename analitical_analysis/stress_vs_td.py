...
data = [e_c, v_c, t_c, e_s, v_s, t_s]

# Defining sputtering temperature
T_min = 100 #ºC
T_max = 500 #ºC
N = T_max - T_min
T_d = np.linspace(T_min, T_max, N)
T_r = 25 # ºC
stress = np.zeros(N)

for i in range(N):
    stress[i], error_c, error_s = thermal_stress(T_r, T_d[i], *data)

stress_paper = np.linspace(214, 1355, N) # [MPa]
...
plt.figure(figsize=(8, 6))
plt.plot(T_d, stress/10**6, label='Calculated stress',color='blue')
plt.plot(T_d, stress_paper, label='Original paper stress',color='red')
plt.title('Variation of Thermal Stress in a range of Deposition Temperature', fontsize=14)
plt.xlabel('Deposition Temperature [ºC]', fontsize=12)
